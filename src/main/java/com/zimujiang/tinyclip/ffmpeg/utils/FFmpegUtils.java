...
import com.zimujiang.tinyclip.ffmpeg.model.VideoResolution;
...
public class FFmpegUtils {
    ...
    private static final String regexDuration_simple = "Duration: (.*?), bitrate: (\\d*) kb\\/s";
    ...
    public static VideoInfo regInfo(String stdout, VideoInfo vi) {
        if (StringUtils.isNotEmpty(stdout)) {
            ...
            if (matcherDuration.find()) {
            	...
            	//如果未匹配到，则尝试wav简版无start信息，主要针对wav
            	 Pattern patternDuration_simple = Pattern.compile(regexDuration_simple);
                 Matcher matcherDuration_simple = patternDuration_simple.matcher(stdout);
                 if (matcherDuration_simple.find()) {
                	 String duration = matcherDuration_simple.group(1);
                     vi.setDuration_f(duration);
                     if (StringUtils.isNotBlank(duration) && duration.indexOf(":") >= 0) {
                         String[] time = duration.split(":");
                         int hour = Integer.valueOf(time[0]);
