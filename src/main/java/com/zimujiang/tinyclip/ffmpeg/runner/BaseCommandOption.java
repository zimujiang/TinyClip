...
public class BaseCommandOption {
	...
	public static final String FFMPEG_M = "/Applications/ffmpeg";
	public static final String FFPROBE_M = "/Applications/ffprobe";
	...
	static {
		...
		if (log.isDebugEnabled()) {
			log.debug("isWindows : '{}'  or isLinux:'{}' oR isMAC DEFAULT_IO_THREADS:'{}'", isWin, isLinux, isMAC,
					DEFAULT_IO_THREADS);
		}
	}
	...
	public static List<String> getFFmpegBinary() {
		if (FFMPEG_BINARY == null) {
			if (isWin) {
				...
				FFMPEG_BINARY = Lists.newArrayList(FFMPEG_M);
			} else if (isLinux) {
	...
	public static List<String> getFFprobeBinary() {
		if (null == FFPROBE_BINARY) {
			if (isWin) {
				...
				FFPROBE_BINARY = Lists.newArrayList(FFPROBE_M);
			} else if (isLinux) {
	...
	public static List<String> toHLSCmdArrays(String input, String m3u8Output, int cutSecond, String tsBaseUrl,
		...
		if (vi != null && vi.getSize() > 0) {
			...
			commands.addAll(getRoateCmdArrays(vi));
			commands.addAll(Lists.newArrayList(CV, FORMAT_LIB264, CA, FORMAT_ACC, STRICT, "-2", F, FORMAT_HLS, THREADS,
					DEFAULT_IO_THREADS.toString(), HLS_TIME, String.valueOf(cutSecond), HLS_LIST_SIZE, "0", HLS_WRAP,
					"0", HLS_BASE_URL, tsBaseUrl, m3u8Output));
	...
	public static List<String> toMP4CmdArrays(String input, String output, VideoInfo vi) {
		if (vi != null && vi.getSize() > 0) {
			List<String> commands = Lists.newArrayList(toInputCommonsCmdArrays(input));
			commands.addAll(getRoateCmdArrays(vi));
			commands.addAll(Lists.newArrayList(CV, FORMAT_LIB264, CA, FORMAT_ACC, STRICT, "-2", THREADS,
					DEFAULT_IO_THREADS.toString(), output));
			return commands;
		}
		return Collections.EMPTY_LIST;
	}
	...
	public static List<String> getRoateCmdArrays(VideoInfo vi) {
		if (vi != null && vi.getSize() > 0) {
			if (vi.getRotate() > 0) {
				...
				return Lists.newArrayList(VF, "transpose=1"
//                        CA,COPY
				);
//                commands.add(VF);
